...
/*   Updated: 2021/03/22 18:41:45 by chgilber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	srand(time(NULL));
	...
	Intern					bob;
	Form	*rff;
	rff = bob.makeForm("shrubbery creation", "roki");
	timmy.SignForm(*rff);
	moi.executeForm(*rff);
	rff = bob.makeForm("Coffeee shop", "Dam");
	rff = bob.makeForm("robotomy request", "takada");
	timmy.SignForm(*rff);
	ichigo.SignForm(*rff);
	ichigo.executeForm(*rff);
	return 0;
}
